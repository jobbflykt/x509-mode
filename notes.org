...
* TODO hexl-mode buffer following x509-asn1-mode line.
Ramains:
- [ ] Test with emacs -Q
- [ ] Test killing hexl buffer and then move around and toggle.
- [ ] Can hexl buffer be made visible automatically if it's not when overlay is
  updated?
- [ ] What happens when several hexl buffers are active? Should overlay list be
